...
export default function ShopClient({ initialProducts, categories }: ShopClientProps) {
  ...
  useEffect(() => {
    ...
    const handleKey = (event: KeyboardEvent) => { if (event.key === 'Escape') setIsFilterDrawerOpen(false); };
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (isOnSale) filtered = filtered.filter((p) => p.isPromo);
    if (isNewArrival) filtered = filtered.filter((p) => p.isNew);
    ...
    switch (sortBy) {
      ...
      default:
        filtered.sort((a, b) => {
          const scoreA = (a.isTrending ? 2 : 0) + (a.isNew ? 1 : 0);
          const scoreB = (b.isTrending ? 2 : 0) + (b.isNew ? 1 : 0);
    ...
    <div className="shop-page">
      ...
      <div className="shop-toolbar">
        ...
        <div className="shop-sort-wrap">
          ...
          <ChevronDown size={16} className="shop-sort-icon" aria-hidden="true" />
        </div>
        ...
        <div className="shop-chips">
          {selectedSizes.map((size) => (
            <span key={size} className="shop-chip">
              Size: {size}
              <button type="button" onClick={() => toggleSize(size)} aria-label={`Remove size ${size} filter`}><X size={13} /></button>
            </span>
          ))}
          {selectedPriceRange !== null && (
            <span className="shop-chip">
              {PRICE_RANGES[selectedPriceRange].label}
              <button type="button" onClick={() => setSelectedPriceRange(null)} aria-label="Remove price filter"><X size={13} /></button>
            </span>
          )}
          {maxPrice !== null && (
            <span className="shop-chip">
              Under K {maxPrice.toLocaleString()}
              <button type="button" onClick={() => setMaxPrice(null)} aria-label="Remove max price filter"><X size={13} /></button>
            </span>
          )}
          {isOnSale && (
            <span className="shop-chip">
              On Sale
              <button type="button" onClick={() => setIsOnSale(false)} aria-label="Remove on sale filter"><X size={13} /></button>
            </span>
          )}
          {isNewArrival && (
            <span className="shop-chip">
              New Arrivals
              <button type="button" onClick={() => setIsNewArrival(false)} aria-label="Remove new arrivals filter"><X size={13} /></button>
            </span>
          )}
          <button type="button" className="shop-chips-clear" onClick={handleClearAll}>Clear all</button>
        </div>
      )}

      <div className="shop-trust hide-mobile">
        <span className="shop-trust-item"><Shield size={14} /> Secure checkout</span>
        <span className="shop-trust-item"><RotateCcw size={14} /> 7-Day Returns</span>
        <span className="shop-trust-item"><Truck size={14} /> Fast delivery</span>
        <span className="shop-trust-item"><MapPin size={14} /> 5 Stores</span>
      </div>

      {filteredProducts.length === 0 ? (
        <div className="shop-empty">
          <Search size={44} className="shop-empty-icon" aria-hidden="true" />
          <h3 className="shop-empty-title">No products found</h3>
          <p className="shop-empty-text">Try a different search or clear your filters.</p>
          <Button variant="primary" onClick={handleClearAll}>Clear Filters</Button>
        </div>
      ) : (
      ...
      <div className={`shop-drawer-overlay ${isFilterDrawerOpen ? 'is-open' : ''}`} onClick={() => setIsFilterDrawerOpen(false)} aria-hidden="true" />
      <div className={`shop-drawer ${isFilterDrawerOpen ? 'is-open' : ''}`} role="dialog" aria-modal="true" aria-label="Filters" aria-hidden={!isFilterDrawerOpen}>
        <div className="shop-drawer-header">
          <h2 className="shop-drawer-heading">Filters</h2>
          <button type="button" className="icon-button" aria-label="Close filters" onClick={() => setIsFilterDrawerOpen(false)}><X size={20} /></button>
        </div>
        <div className="shop-drawer-body">
          ...
          <section className="shop-drawer-section">
            <h3 className="shop-drawer-title">Quick Filters</h3>
            <label className="shop-check"><input type="checkbox" checked={isOnSale} onChange={(e) => setIsOnSale(e.target.checked)} /><span>On Sale</span></label>
            <label className="shop-check"><input type="checkbox" checked={isNewArrival} onChange={(e) => setIsNewArrival(e.target.checked)} /><span>New Arrivals</span></label>
          </section>
          <section className="shop-drawer-section">
            <h3 className="shop-drawer-title">Size</h3>
            <div className="shop-sizes">
              {ALL_SIZES.map((size) => (
                <button key={size} type="button" onClick={() => toggleSize(size)} aria-pressed={selectedSizes.includes(size)} className={`shop-size ${selectedSizes.includes(size) ? 'is-active' : ''}`}>{size}</button>
              ))}
            </div>
          </section>
          <section className="shop-drawer-section shop-drawer-section-last">
            ...
            <div className="shop-price-list">
              {PRICE_RANGES.map((range, index) => (
                <button key={range.label} type="button" onClick={() => setSelectedPriceRange(selectedPriceRange === index ? null : index)} className={`shop-price-item ${selectedPriceRange === index ? 'is-active' : ''}`}><span>{range.label}</span></button>
              ))}
            </div>
          </section>
          <div className="shop-drawer-footer">
            <button type="button" className="btn btn-primary shop-apply" onClick={() => setIsFilterDrawerOpen(false)}>Apply Filters <span className="shop-apply-count">({filteredProducts.length} results)</span></button>
            {activeFilterCount > 0 && <button type="button" className="btn btn-secondary shop-apply" onClick={handleClearAll}><X size={14} /><span>Clear All Filters</span></button>}
          </div>
        </div>
      </div>
    </div>
