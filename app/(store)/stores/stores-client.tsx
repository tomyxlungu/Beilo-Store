...
export default function StoresClient({ initialStores }: StoresClientProps) {
  ...
  const selectedStoreData = initialStores.find(s => s.id === selectedStore) ?? null;
      ...
      <main className="wrap">
        ...
        <div className="grid" style={{ alignItems: 'start' }}>
          ...
          <div className="span-6">
            <div style={{ borderTop: '1px solid var(--urban-fog)' }}>
              {initialStores.map(store => (
                <button
                  ...
                  onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--cloud-veil)'; }}
                ...
                >
                  <div style={{
                    ...
                    height: '40px',
                    borderRadius: '50%',
                    background: selectedStoreData?.id === store.id ? 'var(--charcoal-noir)' : 'var(--cloud-veil)',
                    color: selectedStoreData?.id === store.id ? 'var(--canvas)' : 'var(--charcoal-noir)',
                    display: 'flex',
                    alignItems: 'center',
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
                      <span style={{ fontSize: '12px', color: 'var(--ironclad-grey)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <Clock size={12} />
                        {store.hours}
                      </span>
                      <span style={{ fontSize: '12px', color: 'var(--ironclad-grey)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                  ...
                  <ChevronRight size={18} style={{ color: 'var(--urban-fog)', flexShrink: 0, marginTop: '8px' }} />
                </button>
              ))}
            </div>
          </div>
