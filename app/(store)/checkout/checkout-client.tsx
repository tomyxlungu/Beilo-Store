...
export default function CheckoutClient({ initialStores }: CheckoutClientProps) {
  ...
  const [selectedStore, setSelectedStore] = useState(
    () => getDefaultStore() || initialStores[0]?.name || ''
  );
  ...
  const handleSendOrder = async () => {
    ...
    setSent({ total, method: deliveryMethod, store: selectedStore });
    clearCart();
  };
      ...
      <div className="grid bag-grid">
            ...
            {deliveryMethod === 'pickup' ? (
              <div className="co-stores" role="radiogroup" aria-label="Pickup store">
                {initialStores.map((store) => {
                  ...
                  return (
                    <button key={store.id} type="button" role="radio" aria-checked={active} onClick={() => setSelectedStore(store.name)} className={`co-store ${active ? 'is-active' : ''}`}>
        ...
        <div className="span-4">
          <aside className="bag-summary">
            ...
            <Button variant="primary" size="lg" fullWidth onClick={handleSendOrder} icon={<MessageCircle size={17} />}>Send Order via WhatsApp</Button>
